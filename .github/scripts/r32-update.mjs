...
(async () => {
  ...
  // compute only once standings have SETTLED: if ANY match is live, hold (it will recompute the
  // moment the match finishes via the next scheduled run — exactly when people check their opponent)
  try {
    const sb = await (await fetchTO(SCORE, { headers: { accept: "application/json" } }, { tries: 2, ms: 15000 })).json();
    const liveNow = ((sb && sb.events) || []).some(e => e && e.status && e.status.type && e.status.type.state === "in");
    if (liveNow) { console.log("a match is LIVE - holding; will recompute once it finishes"); return; }
  } catch (e) { console.log("  live-check skipped (" + e.message + "); proceeding"); }
  ...
  // write only when the result actually CHANGED (compute once per change, not on every run)
  try {
    const cur = JSON.parse(await (await fetchTO(READ + "?cb=" + Date.now(), {}, { tries: 2, ms: 15000 })).text());
    if (cur && cur.nations && JSON.stringify(cur.nations) === JSON.stringify(nations)) {
      console.log("no change since last publish (" + cnt + " nations) - skipping write");
