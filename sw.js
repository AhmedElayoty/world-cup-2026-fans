...
// ===== Web Push ===== payload (JSON) sent by the push-send.mjs GitHub Action: {title, body, url, tag, icon}
...
// Tapping a notification: focus an already-open app window, else open one.
self.addEventListener("notificationclick", e => {
  e.notification.close();
  const target = (e.notification.data && e.notification.data.url) || "./index.html";
  e.waitUntil((async () => {
    const wins = await clients.matchAll({ type: "window", includeUncontrolled: true });
    for (const c of wins) { if ("focus" in c) { try { await c.focus(); return; } catch (_) {} } }
